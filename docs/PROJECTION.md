...
Twelve `elide` rules and three `splice` rules remove a sixth of every canonical tree; the exact
figure is generated by `checkNormalization`, which prints it on every run and which `verify.sh`
gates, rather than retyped here where it would go stale at the next pin.
